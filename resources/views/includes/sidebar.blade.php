	...
	<div class="app-sidebar-content" data-scrollbar="true" data-height="100%">
        ...
        <input type="text" class="form-control" placeholder="Sidebar menu filter..." data-sidebar-search="true" />
			...
			@endphp
				...
				<div class="menu-header">Libraries</div>

				@php
					$currentUrl = (Request::path() != '/') ? '/'. Request::path() : '/';

                    function renderLibrariesMenu($value, $currentUrl) {
                        $subMenu = '';
                        $GLOBALS['sub_level'] += 1 ;
                        $GLOBALS['active'][$GLOBALS['sub_level']] = '';
                        $currentLevel = $GLOBALS['sub_level'];
...
$libraries = [];
                    foreach (\App\Library::orderBy('name')->get() as $library) {
						$libraries[] = [
							'icon' => 'fa fa-book',
							'title' => $library->name,
							'url' => '/library/' . $library->slug,
							'route-name' => 'library_' . $library->slug,
						];
					}

                    foreach ($libraries as $key => $menu) {
                        $GLOBALS['parent_active'] = '';
                        ...
                        if (!empty($menu['sub_menu'])) {
                            $GLOBALS['sub_level'] = 0;
                            $subMenu .= '<div class="menu-submenu">';
                            $subMenu .= renderLibrariesMenu($menu['sub_menu'], $currentUrl);
                            $subMenu .= '</div>';
                        }
                        $active = (!empty($menu['route-name']) && (Route::currentRouteName() == $menu['route-name'])) ? 'active' : '';
                        $active = (empty($active) && !empty($GLOBALS['parent_active'])) ? 'active' : $active;
